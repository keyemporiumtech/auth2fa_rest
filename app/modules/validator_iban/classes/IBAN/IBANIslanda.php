...
class IBANIslanda extends IBANGeneric {
    ...
    static function converter($iban) {
        ...
        $obj->bankorg = substr($iban, 8, 2);
        $obj->labelorg_cod = EnumIBANOrg::FRONT_OFFICE;
        $obj->cc = substr($iban, 10, 6);
        $obj->id_national_owner = substr($iban, 16, 10);
        $obj->pattern = "cod_iso3166-0-2|controlnumbers-2-2|bankcod-4-4|bankorg-8-2|cc-10-6|id_national_owner-16-10";
        $obj->length = $LEN;
        $obj->bban = $obj->bankcod . $obj->bankorg . $obj->cc . $obj->id_national_owner;
