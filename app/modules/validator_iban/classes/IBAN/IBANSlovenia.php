...
class IBANSlovenia extends IBANGeneric {
    ...
    static function converter($iban) {
        ...
        $obj->bankcod = substr($iban, 4, 2);
        $obj->bankorg = substr($iban, 6, 3);
        $obj->labelorg_cod = EnumIBANOrg::BRANCH;
        $obj->cc = substr($iban, 9, 8);
        $obj->controlkey = substr($iban, 17, 2);
        $obj->pattern = "cod_iso3166-0-2|controlnumbers-2-2|bankcod-4-2|bankorg-6-3|cc-9-8|controlkey-17-2";
        $obj->length = $LEN;
