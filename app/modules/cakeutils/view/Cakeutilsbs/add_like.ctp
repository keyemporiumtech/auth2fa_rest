<div class="alert alert-success mt-1" role="alert">	
	<code class="text-dark">
	$bs= new TestfkBS();<br/>
	
	<strong style="text-decoration:underline">Aggiungere una condizione di like</strong><br/>
	$bs->addLike("cod", "001", EnumQueryLike::LEFT); <span class="text-danger">// EnumQueryLike [LEFT => %VAL, RIGHT => VAL%, LEFT_RIGHT => %VAL%, PRECISION => VAL]</span><br/>
	
	<br/>
	</code>
</div>

<div class="alert alert-dark mt-1" role="alert">
  	<h4 class="alert-heading">addLike($key, $value, EnumQueryLike)</h4>
	<footer class="blockquote-footer">function <cite title="Source Title">Aggiunge una condizione di LIKE nella where</cite></footer>
</div>

<div class="ml-5 text-success font-weight-bold"><?php debug($all) ?></div>


<div class="mt-2">
	<a href="<?php echo Router::url(array ('controller' => 'cakeutils','action' => 'appGenericBs'))?>">
		<span class="fa fa-arrow-left"></span>
	</a>
</div>