...
class CreditcardValidator {
    ...
    static function creditCardType($number) {
...
// UTILS
    ...
    static function validCard($number, $type) {
        /*
        debug('PATTERN');
        debug(self::validPattern($number, $type));
        debug('LENGTH');
        debug(self::validLength($number, $type));
        debug('LUHN');
        debug(self::validLuhn($number, $type));
         */
        return (self::validPattern($number, $type) && self::validLength($number, $type) && self::validLuhn($number, $type));
    }

    static function validPattern($number, $type) {
        return preg_match(self::$cards[$type]['pattern'], $number);
    }

    static function validLength($number, $type) {
    ...
    static function validCvcLength($cvc, $type) {
    ...
    static function validLuhn($number, $type) {
    ...
    static function luhnCheck($number) {
        $checksum = 0;
