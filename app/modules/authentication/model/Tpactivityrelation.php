...
class Tpactivityrelation extends AppModel {
    ...
    public function afterFind($results, $primary = false) {
        parent::translateValueInField($results, "cod", "title", "tpactivityrelation");
        ...
        foreach ($data as &$obj) {
            if (array_key_exists($this->alias, $obj)) {
                if (!$this->avoidContent && array_key_exists('symbol', $obj[$this->alias])) {
                    $url = WWW_ROOT . "img" . DS . "activityrelation" . DS . "" . $obj[$this->alias]['symbol'];
